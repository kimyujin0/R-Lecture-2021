...
# 6 ok
mpg %>% filter(manufacturer=='audi') %>%group_by(manufacturer) %>% summarise(hwy) %>%arrange(desc(hwy)) %>% head(5)
...
# 8 class별 cty 평균 ok
mpg %>% select(class, cty) %>% group_by(class) %>% summarise(c_avg=mean(cty))

# 9 ok
mpg %>% select(class, cty) %>% group_by(class) %>% summarise(c_avg=mean(cty)) %>% arrange(desc(c_avg))%>% head(7)
