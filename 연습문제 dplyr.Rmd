...
mpg %>%filter(manufacturer %in% c('audi','toyota')) %>%
    ...
    summarise(avg_cty=mean(cty)) %>% arrange(desc(avg_cty))

# toyota의 cty가 평균이 더 높다.
```
...
```{r}
mpg %>% filter(class %in% c('suv','compact')) %>% group_by(class) %>% summarise(avg_cty=mean(cty)) 
...
```{r}
mpg %>% filter(manufacturer=='audi') %>% group_by(manufacturer) %>% 
    summarise(hwy) %>% arrange(desc(hwy)) %>% head(5)
```
