...
gapminder %>% filter(year ==2007) %>% 
    ...
    geom_histogram()                         # 히스토그램이 위로 쌓임

gapminder %>% filter(year ==2007) %>% 
    ggplot(aes(lifeExp,col=continent)) +
    geom_histogram(position = 'dodge')       # 히스토그램이 옆으로 쌓임
...
ggplot(gapminder, aes(gdpPercap, lifeExp, col = continent)) +
    geom_point(alpha = 0.2)   # 데이터가 모여있어 판별하기 쉽지 않음. -> log를 취해줌
ggplot(gapminder, aes(gdpPercap, lifeExp, col = continent)) +
    geom_point(alpha = 0.2) +
    scale_x_log10()

# 4. coord_flip() - x축과 y축을 바꿈
...
gapminder %>% filter(lifeExp > 70) %>% 
    group_by(continent) %>% summarise(n=n_distinct(country)) %>% 
    ggplot(aes(continent, n)) +
    geom_bar(stat = 'identity', aes(fill=continent)) +
    scale_fill_brewer(palette='RdBu')

# 그래프 표시 순서 reorder - 정렬
gapminder %>% filter(lifeExp > 70) %>% 
    group_by(continent) %>% summarise(n=n_distinct(country)) %>% 
